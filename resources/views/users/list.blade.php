  ...
  <div class="px-5 mx-5">
    <div class="row page-header">
      ...
      <span class="float-right page-controller" style="margin-left: auto;">
        <a class="btn btn-outline-success btn-md" href="{{ action('Users\UserController@create') }}"><em class="fa fa-plus-circle"></em> New User</a>
      </span>
    </div>
  </div>
          ...
          <tbody>
            @foreach ($data['users'] as $user)
              <tr>
                ...
                <td>{{ $user->domain }}</td>
                <td>{{ $user->company }}</td>
                <td><span class="flag-icon flag-icon-{{ strtolower($user->country) }} border"></span> {{ $user->location }}</td>
                ...
                <td>
                    <a class="btn btn-outline-info btn-sm o-button" href="{{ url('users/show/'.$user->id) }}"><span class="icon-holder"><em class="ti-eye"></em></span> View</a>
                </td>
              </tr>
            @endforeach
